...
int main(int argc, char *argv[])
{
   ...
   */
	...
	while (1) {
		...
		case 'a':	/*angle */
			if (!isdigit((int)*optarg)) {
				fprintf(stderr,
					_
					("[Error] The thing behind -a has to be a number! \n"));
				exit(1);
			}
			...
			if (angle < 0) {
				fprintf(stderr,
					_
					("[Hint] Um, you set angle to 0, try 1 instead ;-)\n"));
				exit(1);
			}
			break;
		case 'b':	/*size to skip from the beginning (beginning-offset) */
			if (!isdigit((int)*optarg)) {
				fprintf(stderr,
					_
					("[Error] The thing behind -b has to be a number! \n"));
				exit(1);
			}
			...
			if (!size_suffix) {
				fprintf(stderr,
					_
					("[Error] Wrong suffix behind -b, only b,k,m or g \n"));
				exit(1);
			}
		...
		case 'c':	/*chapter *//*NOT WORKING!! */
			if (!isdigit((int)*optarg)) {
				fprintf(stderr,
					_
					("[Error] The thing behind -c has to be a number! \n"));
				exit(1);
			}
		...
		case 'e':	/*size to stop from the end (end-offset) */
			if (!isdigit((int)optarg[0])) {
				fprintf(stderr,
					_
					("[Error] The thing behind -e has to be a number! \n"));
				exit(1);
			}
			temp_var = atol(optarg);
			size_suffix = strpbrk(optarg, "bkmgBKMG");
		...
		case 'i':	/*input dir, if the automatic needs to be overridden */
			if (isdigit((int)*optarg)) {
				fprintf(stderr,
					_
					("[Error] Erm, the number comes behind -n ... \n"));
				exit(1);
			}
			...
			safestrncpy(provided_input_dir, optarg,
				    sizeof(provided_input_dir) - 1);
		...
		case 'n':	/*title number */
			if (!isdigit((int)*optarg)) {
				fprintf(stderr,
					_
					("[Error] The thing behind -n has to be a number! \n"));
				exit(1);
			}
		...
		case 'o':	/*output destination */
			if (isdigit((int)*optarg))
				fprintf(stderr,
					_
					("[Hint] Erm, the number comes behind -n ... \n"));
		...
		case '4':
			if (alternate_dir_count < options_char - 48) {
				fprintf(stderr,
					_
					("[Error] Please specify output dirs in this order: -o -1 -2 -3 -4 \n"));
				exit(1);
			}
			...
			safestrncpy(alternate_output_dir[options_char - 49],
				    optarg,
				    sizeof(alternate_output_dir
					   [options_char - 49]) - 1);
			provided_output_dir_flag = TRUE;
			alternate_dir_count++;
			force_flag = TRUE;
		...
		case 't':	/*provided title instead of the one from dvd,
			...
			safestrncpy(provided_dvd_name, optarg,
				    sizeof(provided_dvd_name) - 1);
			provided_dvd_name_flag = TRUE;
			if (!strcasecmp(provided_dvd_name, "stdout")
			    || !strcasecmp(provided_dvd_name, "-")) {
				stdout_flag = TRUE;
				force_flag = TRUE;
			}
		...
		case 'w':	/*sets a watchdog timer to cap the amount of time spent
			...
			if (!isdigit((int)*optarg)) {
				fprintf(stderr,
					_
					("[Error] The thing behind -w has to be a number! \n"));
				exit(1);
			}
			...
			if (watchdog_minutes < 1) {
				fprintf(stderr,
					_
					("[Hint] Negative minutes aren't allowed - disabling watchdog.\n"));
				watchdog_minutes = 0;
			}
			break;
		...
		case 'F':	/*Fast-switch */
			if (!isdigit((int)*optarg)) {
				fprintf(stderr,
					_
					("[Error] The thing behind -F has to be a number! \n"));
				exit(1);
			}
			sscanf(optarg, "%i", &fast_factor);

			/*atm is BLOCK_COUNT == 64 */
			if (fast_factor > BLOCK_COUNT) {
				fprintf(stderr,
					_
		...
		case 'V':	/*version number output */
			printf("Vobcopy " VERSION
			       " - GPL Copyright (c) 2001 - 2009 robos@muon.de\n");
			exit(0);
			break;
		...
		case '-':	/* no getopt, complain */
			fprintf(stderr,
				_
				("[Error] %s was compiled without support for long options.\n"),
				argv[0]);
			usage(argv[0]);
			exit(1);
			break;
	...
	if (quiet_flag) {
		...
		fprintf(stderr,
			_
			("[Hint] Quiet mode - All messages will now end up in %s\n"),
			tmp_path);
		if ((temp =
		     open(tmp_path, O_RDWR | O_CREAT | O_EXCL, 0666)) == -1) {
			if (errno == EEXIST) {
				if (!force_flag) {
					...
					printf
					    ("[Warning] Overwriting %s as requested with -f\n",
					     tmp_path);
				}
			} else {
				printf
				    ("[Error] Aaah! Re-direct of stderr to %s didn't work! If -f is not used I stop here... \n",
				     tmp_path);
				printf
				    ("[Hint] Use -f to continue (at your risk of stupid ascii text ending up in your VOBs)\n");
				if (!force_flag)
					exit(1);
			}
		} else {
		...
		if (chmod(tmp_path, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH) ==
		    -1) {
			printf("[Error] Error: %s\n", strerror(errno));
			printf
			    ("[Error] Changing mode of %s didn't work! If -f is not used I stop here... \n",
			     tmp_path);
			printf
			    ("[Hint] Use -f to continue (at your risk of stupid ascii text ending up in your VOBs)\n");
			if (!force_flag)
	...
	/* this here starts writing the logfile */
	if (verbosity_level > 1) {
		...
		if ((temp =
		     open(logfile_path, O_RDWR | O_CREAT | O_EXCL,
			  0666)) == -1) {
			printf("[Error] Error: %s\n", strerror(errno));
			if (errno == EEXIST) {
				printf
		...
		if (chmod(logfile_path, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH)
		    == -1) {
			printf("[Error] Error: %s\n", strerror(errno));
			printf
			    ("[Error] Changing mode of %s didn't work! If -f is not used I stop here... \n",
	...
	/* there is still the path as in vobcopy-0.2.0 */
	if (optind < argc) {
		provided_input_dir_flag = TRUE;
		...
		safestrncpy(provided_input_dir, argv[optind],
			    sizeof(provided_input_dir) - 1);
	}

	/*the path has been given to us */
	if (provided_input_dir_flag) {
		...
		if ((dvd_count =
		     get_device_on_your_own(provided_input_dir,
					    dvd_path)) <= 0) {
			fprintf(stderr,
				_
	...
	if (!mounted) {
		...
		if (!(dvd = DVDOpen(dvd_path))) {
			...
			if (dvd_count > 1)
				fprintf(stderr,
					_
					("[Hint] By the way, you have %i cdroms|dvds mounted, that probably caused the problem\n"),
					dvd_count);
			DVDClose(dvd);
			exit(1);
		}
		...
		if (provided_dvd_name_flag)
			safestrncpy(dvd_name, provided_dvd_name,
				    sizeof(dvd_name) - 1);
		else
		...
		for (i = 0; i < tt_srpt->nr_of_srpts; i++) {
			...
			if (i > 0) {
				if (tt_srpt->title[i].nr_of_ptts >
				    tt_srpt->title[most_chapters].nr_of_ptts)
					most_chapters = i;
			}
		}

		/*no title specified (-n ) */
		if (longest_title_flag) {
		...
		fprintf(stderr,
			_
			("[Info] Most chapters has title %i with %d chapters.\n"),
			(most_chapters + 1),
			tt_srpt->title[most_chapters].nr_of_ptts);
		...
		if (info_flag) {
			...
			for (i = 0; i < tt_srpt->nr_of_srpts; i++) {
				...
				if (chapters > 1)
					fprintf(stderr,
						_
						("[Info] Title %i has %d chapters.\n"),
						(i + 1), chapters);
				else
					fprintf(stderr,
						_
						("[Info] Title %i has %d chapter.\n"),
						(i + 1), chapters);
			}
		}

		if (chapid < 0
		    || chapid >= tt_srpt->title[titleid - 1].nr_of_ptts) {
		...
		fprintf(stderr,
			...
			sum_angles);
		if (angle < 0
		    || angle >= tt_srpt->title[titleid - 1].nr_of_angles) {
		...
		if (info_flag) {
			...
			for (i = 0; i < tt_srpt->nr_of_srpts; i++) {
				...
				if (angles > 1)
					fprintf(stderr,
						_
						("[Info] Title %i has %d angles.\n"),
						(i + 1), angles);
				else
					fprintf(stderr,
						_
						("[Info] Title %i has %d angle.\n"),
						(i + 1), angles);
			}

			disk_vob_size =
			    get_used_space(provided_input_dir, verbosity_level);
		}

		/*get the whole vob size via stat( ) manually */
		if (mounted && !mirror_flag) {
			vob_size = get_vob_size(titleid, provided_input_dir);
			vob_size -= seek_start * 2048;
			vob_size -= stop_before_end * 2048;
		...
		/*this gets the size of the whole dvd */
		if (mirror_flag) {
		...
		if (mirror_flag) {
			...
			if ((force_flag || pwd_free > disk_vob_size)
			    && alternate_dir_count < 2) {
				...
				if (!stdout_flag) {
					...
					fprintf(stderr,
						_
						("[Info] Writing files to this dir: %s\n"),
						name);
				}
 ...
 next:				/*for the goto - ugly, I know... */
					...
					if (strstr(directory->d_name, ".vob") ||
							...
							for (a = 1; a < subvob;
								...
								if (verbosity_level > 1)
									fprintf
									    (stderr,
									     _
									     ("[Info] Vob %d %d (%s) has a size of %lli\n"),
									     title_nr,
									     subvob,
									     input_file,
									     buf.
						...
						for (i = start;
							...
							if (verbosity_level >= 1
								...
								fprintf(stderr,
									_
									("[Warn] Had to skip (couldn't read) %d blocks (before block %d)! \n "),
									skipped_blocks,
									i);
				...
				if (overall_skipped_blocks > 0)
					fprintf(stderr,
						_
						("[Info] %d blocks had to be skipped, be warned.\n"),
						overall_skipped_blocks);
				exit(0);
			} else {
				fprintf(stderr,
		...
		fprintf(stderr,
			_("[Info] Title has %d chapters and %d angles\n"),
			tt_srpt->title[titleid - 1].nr_of_ptts,
			tt_srpt->title[titleid - 1].nr_of_angles);
	 ...
	 */
		vts_file =
		    ifoOpen(dvd, tt_srpt->title[titleid - 1].title_set_nr);
		if (!vts_file) {
			fprintf(stderr,
				_
				("[Error] Can't open the title %d info file.\n"),
	 ...
	 */
		dvd_file =
		    DVDOpenFile(dvd, tt_srpt->title[titleid - 1].title_set_nr,
				DVD_READ_TITLE_VOBS);
		if (!dvd_file) {
			fprintf(stderr,
				_
				("[Error] Can't open title VOBS (VTS_%02d_1.VOB).\n"),
				tt_srpt->title[titleid - 1].title_set_nr);
			ifoClose(vts_file);
			ifoClose(vmg_file);
			DVDClose(dvd);
		...
		if (vob_size ==
			...
			vob_size =
			    ((off_t) (file_size_in_blocks) *
			     (off_t) DVD_VIDEO_LB_LEN) - (seek_start * 2048) -
			    (stop_before_end * 2048);
			if (verbosity_level >= 1)
		...
		if (pwd_free < vob_size) {
			...
			if (pwd_free == 0 && !force_flag) {
				if (verbosity_level > 1)
					fprintf(stderr,
						_
						("[Error] Hmm, statfs (statvfs) seems not to work on that directory. \n"));
				fprintf(stderr,
					_
					("[Error] Hmm, statfs (statvfs) seems not to work on that directory. \n"));
				fprintf(stderr,
					_
					("[Hint] Nevertheless, do you want vobcopy to continue [y] or do you want to check for \n"));
				fprintf(stderr,
					_("[Hint] enough space first [q]?\n"));
				...
				while (1) {
					...
					if (op == 'y') {
						...
						fprintf(stderr,
							_
							("[Error] Please choose [y] to continue or [n] to quit\n"));
					}
				}
			}
	 ...
	 *********************/
		...
		while (offset <
		       (file_size_in_blocks - seek_start - stop_before_end)) {
			partcount++;
			...
			if (!stdout_flag) {
				...
				if (paths_taken == 0) {
					...
					if (verbosity_level > 1)
						fprintf(stderr,
							_
							("[Info] Free space for -o dir: %.0f\n"),
							(float)free_space);
					if (large_file_flag)
						make_output_path(pwd, name,
								 get_dvd_name_return,
								 dvd_name,
								 titleid, -1);
					else
				...
				/*The magic number 2147473408 is to check for 2GBs of disk space */
				/*here the output size gets adjusted to the given free space */
				if (!large_file_flag && force_flag
				    && free_space < 2147473408) {
					space_greater_2gb_flag = FALSE;
					max_filesize_in_blocks = ((free_space - 2097152) / 2048);	/* - 2 MB */
					if (verbosity_level > 1)
						fprintf(stderr,
							_
							("[Info] Taken max_filesize_in_blocks(2GB version): %.0f\n"),
							(float)
					...
					if (verbosity_level > 1)
						fprintf(stderr,
							_
							("[Info] Taken max_filesize_in_blocks(lfs version): %.0f\n"),
							(float)
							max_filesize_in_blocks);
					paths_taken++;
				} else if (!large_file_flag) {
					/*if free_space is more than  2 GB fall back to max_filesize_in_blocks=2GB */
					max_filesize_in_blocks = 1048571;
...
#endif
				{
					...
					if (get_free_space
					    (name, verbosity_level) < 2097152) {
						fprintf(stderr,
							_
							("[Error] Seems your platter is full...\n"));
						exit(1);
					}
			...
			starttime = time(NULL);
			for (;
			     (offset + (off_t) seek_start) <
			     ((off_t) file_size_in_blocks -
			      (off_t) stop_before_end)
			     && offset - (off_t) max_filesize_in_blocks_summed -
				...
				progressUpdate(starttime, (int)offset / 512,
			...
			if (!stdout_flag) {
				...
				if (verbosity_level >= 1) {
					fprintf(stderr,
						_
						("[Info] max_filesize_in_blocks %8.0f \n"),
						(float)max_filesize_in_blocks);
					fprintf(stderr,
						_
						("[Info] offset at the end %8.0f \n"),
						(float)offset);
					fprintf(stderr,
						_
						("[Info] file_size_in_blocks %8.0f \n"),
				...
				if (verbosity_level >= 1) {
					fprintf(stderr,
						_
						("[Info] Single file size (of copied file %s ) %.0f\n"),
						name, (float)buf.st_size);
					fprintf(stderr,
						_
						("[Info] Cumulated size %.0f\n"),
						(float)disk_vob_size);
				}
			}
			...
			fprintf(stderr,
				_("[Info] Successfully copied file %s\n"),
				name);
		...
		fprintf(stderr,
			_
			("[Info] Combined size of title-vobs: %.0f (%.0f MB)\n"),
			(float)vob_size, (float)vob_size / (1024 * 1024));
		fprintf(stderr,
			_
			("[Info] Copied size (size on disk):  %.0f (%.0f MB)\n"),
			(float)disk_vob_size,
			(float)disk_vob_size / (1024 * 1024));
		if ((vob_size - disk_vob_size) > MAX_DIFFER) {
			fprintf(stderr,
				_
				("[Error] Hmm, the sizes differ by more than %d\n"),
				MAX_DIFFER);
			fprintf(stderr,
				_
 ...
 */
	...
	int add_end_slash(char *path) {	/* add a trailing '/' to path */
		char *pointer;
 ...
 */
	off_t get_free_space(char *path, int verbosity_level) {
		...
		if (verbosity_level >= 1) {
			fprintf(stderr,
				_
				("[Info] In freespace_getter:for %s : %.0f free\n"),
				path, (float)sum);
			fprintf(stderr,
				_
				("[Info] In freespace_getter:bavail %ld * bsize %ld = above\n"),
				temp1, temp2);
		}
 ...
 */
	...
	off_t get_used_space(char *path, int verbosity_level) {
		...
		if (verbosity_level >= 1) {
			fprintf(stderr,
				_
				("[Info] In usedspace_getter:for %s : %.0f used\n"),
				path, (float)sum);
			fprintf(stderr,
				_
				("[Info] In usedspace_getter:part1 %ld, part2 %ld\n"),
				temp1, temp2);
		}
...
#if defined( HAS_LARGEFILE ) || defined ( MAC_LARGEFILE )
		fprintf(stderr,
			_("[-l (large-file support for files > 2GB)] \n"));
#endif
		exit(1);
	}
	...
	void re_name(char *output_file) {
		...
		if (!link(output_file, new_output_file)) {
			if (unlink(output_file)) {
				fprintf(stderr,
					_
					("[Error] Could not remove old filename: %s \n"),
					output_file);
				fprintf(stderr,
					_
					("[Hint] This: %s is a hardlink to %s. Dunno what to do... \n"),
			...
			if (errno == EEXIST && !overwrite_flag) {
				fprintf(stderr,
					_
					("[Error] File %s already exists! Gonna name the new one %s.dupe \n"),
			...
			/*EPERM means that the filesystem doesn't allow hardlinks, e.g. smb */
			if (errno == EPERM) {
		...
		if (mkdir(name, 0777)) {
			if (errno == EEXIST) {
				if (!overwrite_all_flag) {
					...
					fprintf(stderr,
						_
						("[Hint] You can either [c]ontinue writing to it, [x]overwrite all or you can [q]uit: "));
				}
...
*/
	...
	int progressUpdate(int starttime, int cur, int tot, int force) {
		...
		if (progress_time == 0 || progress_time != time(NULL) || force) {
			int barLen, barChar, numChars, timeSoFar, minsLeft,
			    secsLeft, ctr, cols;
			...
			barChar = tot / barLen;
			percentComplete =
			    (float)((float)cur / (float)tot * 100.0);
...
/* 	   guess remaining time */
			...
			if (percentComplete == 0)
				...
				timePerPercent =
				    (float)(timeSoFar / percentComplete);
